...
LOCAL = "https://html-render.herokuapp.com/"
...
class Building:
    def __init__(self, html: str, css: str, js: str) -> None:
        ...
        """
        self.id = uuid.uuid4()
        if [*map(type, (html, css, js))] == [str, str, str]:
            with open(f"./public/builds/{self.id}.html", "w") as f:
                with open("./src/template.html", "r") as template:
                    template = template.read()
                    f.write(template.format(id=self.id, css=css, html=html, js=js))

    def convert(self, format_: str, type_: str, selector: str, size: list[str, str], timeout: float) -> bool | str:
        """
        ...
        filename = f"{self.id}.{format_}"
        url = f"{LOCAL}/api/v1/builds/{self.id}.html"
    ...
    @classmethod
    def convertURL(cls, url: str, format_: str, type_: str, selector: str, size: list[str, str], timeout: float) -> bool | str:
        """
